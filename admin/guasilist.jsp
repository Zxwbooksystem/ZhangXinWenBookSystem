...
<%	 CommDAO dao = new CommDAO(); 
     String sql = "select * from guasi where 1=1";
	 String url = "/tushupiaoliusys/admin/guasilist.jsp?1=1";
                ...
                <div class="block">
                    ...
                    <thead>
						<tr>
							<th colspan="9" align="left">
								<form action="/tushupiaoliusys/admin/guasilist.jsp?f=f" method="post" class="form-inline" style="padding-bottom: 10px;">
					...
					<tbody>
					 <%
					   ...
					   if(did!=null){
						   dao.commOper("delete from guasi where id="+did);
					   }
					   ...
					   ArrayList<HashMap> list=(ArrayList<HashMap>)bean.getCollection();
						...
						<td><%=xxmap.get("bianhao") %></td>
				          <td><%=jcmap.get("name") %></td>
				           <td><%=jcmap.get("dinjia") %> 元</td>
				           <td><%=map.get("fkzt") %> </td>
				           <td><%=map.get("fkfs") %> </td>
				          <td><%=map.get("savetime") %></td>
							<td align="center">
							<%if(map.get("fkzt").equals("已付款")){%>
							<a href="<%=path %>/admin/guasilist.jsp?did=<%=map.get("id") %>" >删除记录</a>
							<%}%>
							</td>
						</tr>
						<%} %>
