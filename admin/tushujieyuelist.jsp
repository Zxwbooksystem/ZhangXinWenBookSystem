...
<%	 CommDAO dao = new CommDAO(); 
     String sql = "select * from tushujy where 1=1 and delstatus='0' and shstatus='' and gsstatus=''";
	 String url = "/tushupiaoliusys/admin/tushujieyuelist.jsp?1=1";
                ...
                <div class="block">
                    ...
                    <thead>
						<tr>
							<th colspan="9" align="left">
								<form action="/tushupiaoliusys/admin/tushujieyuelist.jsp?f=f" method="post" class="form-inline" style="padding-bottom: 10px;">
									<input class="input-xlarge" placeholder="借阅人姓名..." id="key" name="key" type="text" value="<%=key %>">
					...
					<tbody>
					 <%
					   ...
					   if(did!=null){
						   dao.commOper("update tushujy set delstatus=1 where id="+did);
					   }
					   ...
					   ArrayList<HashMap> list=(ArrayList<HashMap>)bean.getCollection();
					   	for(HashMap jymap:list){
					   		HashMap xxmap = dao.select("select * from tushuxx where id="+jymap.get("tsxxid")).get(0);
						...
						<td><%=mmm.get("tname") %>  <%=mmm.get("tel") %></td>
						<td><%=xxmap.get("bianhao") %></td>
				         <td><%=map.get("name") %></td>
				         <td><%=map.get("cbs") %></td>
				          <td><%=map.get("shujia") %></td>
				          <td><%=fprotype.get("typename") %> （<%=sprotype.get("typename") %>）</td>
						   <td><%=jymap.get("hsstatus") %> </td>
						    <td><%=jymap.get("yjhstime") %> </td>
							<td><%=jymap.get("savetime") %> </td>
							<td align="center">
							<%  DateFormat df = new SimpleDateFormat("yyyy-MM-dd");
					          Date dt1 = df.parse(jymap.get("yjhstime").toString());
					          Date dt2 = df.parse(Info.getDateStr());
							   if(dt1.getTime() < dt2.getTime()){%>
								<a href="<%=path %>/admin/fukuanadd.jsp?jieyueid=<%=jymap.get("id") %>" style="color: red;">罚款通知</a>
								<%} else{%>
							<a href="<%=path %><%=path %>?ac=hstg&tsid=<%=map.get("id") %>&id=<%=jymap.get("id") %>" >归还登记</a>
							<%}%>
