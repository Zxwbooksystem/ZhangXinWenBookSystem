...
<%	 CommDAO dao = new CommDAO(); 
     String sql = "select * from fakuan where 1=1";
	 String url = "/tushupiaoliusys/admin/fakuanlist.jsp?1=1";
                ...
                <div class="block">
                    ...
                    <thead>
						<tr>
							<th colspan="9" align="left">
								<form action="/tushupiaoliusys/admin/fakuanlist.jsp?f=f" method="post" class="form-inline" style="padding-bottom: 10px;">
					...
					<tbody>
					 <%
					   ...
					   if(did!=null){
						   dao.commOper("delete from fakuan where id="+did);
					   }
					   ...
					   ArrayList<HashMap> list=(ArrayList<HashMap>)bean.getCollection();
					   	for(HashMap map:list){
					   	 HashMap mmmap = dao.select("select * from member where id="+map.get("mid")).get(0);
					   	  HashMap xxmap = dao.select("select * from tushuxx where id="+map.get("tsxxid")).get(0);
				    	  HashMap jcmap = dao.select("select * from tushu where id="+xxmap.get("tsid")).get(0);
						    %>
						<tr  align="center" height="20px;">
						<td> <%=mmmap.get("tname") %>  <%=mmmap.get("tel") %> 
						</td>
						<td><%=xxmap.get("bianhao") %></td>
				          <td><%=jcmap.get("name") %></td>
				           <td><%=map.get("fkje") %> 元</td>
				           <td><%=map.get("fkzt") %> </td>
				           <td><%=map.get("fkfs") %> </td>
				           <td><%=map.get("note") %> </td>
				          <td><%=map.get("savetime") %></td>
							<td align="center">
							<%if(map.get("fkzt").equals("已付款")){%>
							<a href="<%=path %>/admin/fakuanlist.jsp?did=<%=map.get("id") %>" >删除记录</a>
							<%}%>
							</td>
						</tr>
						<%} %>
