...
<%	 CommDAO dao = new CommDAO(); 
     String sql = "select * from tushujy where 1=1 and delstatus='0' and shstatus='待审核'";
	 String url = "/tushupiaoliusys/admin/huansshhlist.jsp?1=1";
                    ...
                    <thead>
						<tr>
							<th colspan="9" align="left">
								<form action="/tushupiaoliusys/admin/huansshhlist.jsp?f=f" method="post" class="form-inline" style="padding-bottom: 10px;">
				         ...
				         <td><%=map.get("cbs") %></td>
				          ...
				          <td><%=fprotype.get("typename") %> （<%=sprotype.get("typename") %>）</td>
						   <td><%=jymap.get("hsstatus") %> </td>
						    <td><%=jymap.get("shstatus") %> </td>
							<td><%=jymap.get("savetime") %> </td>
							<td><%=jymap.get("hstime") %> </td>
							<td align="center">
							<a href="<%=path %><%=path %>?ac=hstg&id=<%=jymap.get("id") %>&tsxxid=<%=jymap.get("tsxxid") %>" >确认</a>
							</td>
						</tr>
						<%} %>
						<tr class="odd gradeX">
